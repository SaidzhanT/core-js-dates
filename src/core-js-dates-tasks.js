...
function getNextFriday(date) {
  ...
  const diff = (5 - date.getUTCDay() + 7) % 7;
  if (diff === 0) {
...
function getWeekNumberByDate(date) {
  ...
  let dayNumber = Math.floor(
    (dateCopy - yearStart) / (24 * 60 * 60 * 1000) + 1
  );
  const startWeekDay = yearStart.getUTCDay() || 7;

  if (startWeekDay !== 1) {
    dayNumber += startWeekDay - 1;
  }
...
function getQuarter(date) {
  ...
  if (month > 11) {
    throw new Error(
      'Invalid month: Month must be between 0 (January) and 11 (December)'
    );
  }
